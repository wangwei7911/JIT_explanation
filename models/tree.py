...
class Node(object):
    ...
    def build(self, tree):
        ...
        self.prob = self.forward(tree)    #选择右分支的概率
        if not self.isLeaf:
    ...
    def get_penalty(self, tree):
        ...
        self.penalty = (- 0.5 * tf.math.log(self.alpha + tree.eps)
                        - 0.5 * tf.math.log(1. - self.alpha + tree.eps))
        # Replace possible NaN values with zeros
        self.penalty = tf.where(
            tf.math.is_nan(self.penalty), tf.zeros_like(self.penalty), self.penalty)
        return self.penalty

    def get_loss(self, y, tree):
        if self.isLeaf:
            # Cross-entropies (batch) of soft labels with output of this leaf
            leaf_ce = - tf.reduce_sum(y * tf.math.log(tree.eps + self.prob), axis=1)
...
class SoftBinaryDecisionTree(object):
    ...
    def initialize_variables(self, sess, x, batch_size):
        ...
        ema_vars = [v for v in tf.compat.v1.global_variables() if
                    'ExponentialMovingAverage' in v.name and
                    'Const' not in v.name]
        independent_vars = [v for v in tf.compat.v1.global_variables() if
                            v not in ema_vars]
        feed_dict = {self.input_layer: x[:batch_size]}

        init_indep_vars_op = tf.compat.v1.variables_initializer(independent_vars)
        init_ema_vars_op = tf.compat.v1.variables_initializer(ema_vars)
    ...
    def predict_proba(self,x):
        if self.model and self.initialized:
            return self.model.predict_proba(x, verbose=1)
        else:
            print('Missing initialized model instance.')
    ...
    def predict_generator(self,x):
        if self.model and self.initialized:
            return self.model.predict_generator(x, verbose=1)
        else:
            print('Missing initialized model instance.')